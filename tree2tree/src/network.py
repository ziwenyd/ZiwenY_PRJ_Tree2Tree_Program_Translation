...
class Seq2SeqModel(nn.Module):
    def __init__(self,
        ...
        self.decoder = nn.LSTM(input_size=self.embedding_size, hidden_size=self.hidden_size,
                               num_layers=self.num_layers, batch_first=True, dropout=self.dropout_rate)

        self.attention_encoder_linear = nn.Linear(
            self.hidden_size, self.hidden_size)
        self.attention_decoder_linear = nn.Linear(
            self.hidden_size, self.hidden_size)
        ...
        self.optimizer = optim.RMSprop(
            self.parameters(), lr=self.learning_rate, alpha=0.95)
    ...
    def attention(self, encoder_outputs, decoder_output):
        ...
        res = self.attention_tanh(self.attention_encoder_linear(
            encoder_attention) + self.attention_decoder_linear(decoder_output))
        return res
    ...
    def decode(self, encoder_outputs, encoder_state, decoder_inputs, feed_previous, init_start_tokens=None):
        ...
        if init_start_tokens is not None:
            ...
            start_tokens = self.decoder_embedding(
                decoder_inputs[:, 0].unsqueeze(1))
        if feed_previous:
            states = []
            ...
            for time_step in xrange(1, self.max_target_len):
                ...
                embedding = self.decoder_embedding(
                    decoder_input.squeeze().unsqueeze(0))
    ...
    def forward(self, encoder_inputs, decoder_inputs, feed_previous=False):
        ...
        predictions, _ = self.decode(
            encoder_outputs, encoder_state, decoder_inputs, feed_previous)
        return predictions
    ...
    def get_batch(self, data, start_idx):
        ...
        for i in xrange(self.batch_size):
            ...
            encoder_pad = [data_utils.PAD_ID] * \
                (self.max_source_len - len(encoder_input))
...
class Seq2TreeModel(Seq2SeqModel):
    ...
    def build_prediction_node(self, current_prediction, start_token, target_node, decoder_input, idx_batch, depth, parent):
        node = {'prediction': current_prediction, 'start_token': start_token, 'target_node': target_node,
                'value': decoder_input, 'idx_batch': idx_batch, 'depth': depth, 'parent': parent, 'children': []}
        return node
    ...
    def forward(self, encoder_inputs, init_decoder_inputs, feed_previous=False):
        ...
        default_start_token = self.decoder_embedding(
            default_start_token.unsqueeze(0)).squeeze().unsqueeze(0)

        for idx in xrange(len(init_decoder_inputs)):
            current_target = init_decoder_inputs[idx]
            current_node = self.build_prediction_node(
                [], default_start_token, current_target, current_target['value'], current_target['idx_batch'], current_target['depth'], None)
            prediction_nodes.append(current_node)
            queue.append(((encoder_state[0][:, idx, :].unsqueeze(
                1), encoder_state[1][:, idx, :].unsqueeze(1)), idx))
        ...
        while head < len(queue):
            ...
            for node_idx in decoder_inputs_node_idx:
                decoder_inputs.append(
                    prediction_nodes[node_idx]['value'].unsqueeze(0))
                encoder_outputs.append(
                    init_encoder_outputs[prediction_nodes[node_idx]['idx_batch']].unsqueeze(0))
                start_tokens.append(
                    prediction_nodes[node_idx]['start_token'].unsqueeze(0))
            ...
            predictions_logits, states = self.decode(
                encoder_outputs, (init_h_states, init_c_states), decoder_inputs, feed_previous, start_tokens)
            ...
            for idx_node in xrange(len(decoder_inputs_node_idx)):
                ...
                for time_step in xrange(self.max_target_len - 1):
                    current_predictions.append(
                        predictions[time_step][idx_node])
                ...
                if feed_previous == False:
                    ...
                    for idx in xrange(self.max_target_len - 1):
                        if prediction_nodes[num_node]['value'][idx + 1].item() == data_utils.NT_ID:
                            ...
                            new_node = self.build_prediction_node(
                                [], states[idx][0][self.num_layers - 1:, idx_node, :], new_target, new_target['value'], new_target['idx_batch'], new_target['depth'], num_node)
                            prediction_nodes.append(new_node)
                            prediction_nodes[num_node]['children'].append(
                                len(prediction_nodes) - 1)
                            queue.append(((states[idx][0][:, idx_node, :].unsqueeze(
                                1), states[idx][1][:, idx_node, :].unsqueeze(1)), len(prediction_nodes) - 1))
                            child_idx += 1
                else:
                    child_idx = 0
                    for idx in xrange(self.max_target_len - 1):
                        ...
                        if prediction_nodes[num_node]['prediction'][idx].item() == data_utils.NT_ID:
                            if current_target == None or child_idx >= len(current_target['children']):
                                default_value = [
                                    data_utils.GO_ID, data_utils.EOS_ID] + [data_utils.PAD_ID] * (self.max_target_len - 2)
                                default_value = Variable(
                                    torch.LongTensor(default_value))
                                if self.cuda_flag:
                                    default_value = default_value.cuda()
                                new_node = self.build_prediction_node([], states[idx][0][self.num_layers - 1:, idx_node, :], None, default_value,
                                                                      prediction_nodes[num_node]['idx_batch'], prediction_nodes[num_node]['depth'] + 1, num_node)
                            else:
                                new_target = current_target['children'][child_idx]
                                new_node = self.build_prediction_node(
                                    [], states[idx][0][self.num_layers - 1:, idx_node, :], new_target, new_target['value'], new_target['idx_batch'], new_target['depth'], num_node)
            ...
            if item.item() == data_utils.EOS_ID:
                ...
                if child_idx >= len(current_children):
                    prediction = prediction + \
                        [data_utils.LEFT_BRACKET_ID, data_utils.RIGHT_BRACKET_ID]
                else:
                    prediction = prediction + \
                        self.tree2seq(prediction_nodes,
    ...
    def get_batch(self, data, start_idx):
        ...
        for i in xrange(self.batch_size):
            ...
            encoder_inputs.append(list(reversed(encoder_input + encoder_pad)))
            decoder_inputs.append(
                self.build_data_node(decoder_input, i, 0, None))
...
class Tree2SeqModel(nn.Module):
    ...
    def decode(self, encoder_outputs, attention_masks, encoder_state, decoder_inputs, feed_previous):
        ...
        start_tokens = self.decoder_embedding(
            decoder_inputs[:, 0].unsqueeze(1))
        attention_output = Variable(torch.zeros(
            decoder_inputs.size()[0], self.hidden_size))
        if self.cuda_flag:
        ...
        if feed_previous:
            ...
            for time_step in xrange(1, self.max_target_len):
                ...
                if len(output_squeeze.size()) == 1:
                    output_squeeze = output_squeeze.unsqueeze(0)
                prediction, attention_output = self.predict(
                    output_squeeze, encoder_outputs, attention_masks)
                decoder_input = prediction.max(1)[1]
                embedding = self.decoder_embedding(
                    decoder_input.squeeze().unsqueeze(0))
                embedding = torch.transpose(embedding, 0, 1)
    ...
    def forward(self, encoder_managers, decoder_inputs, feed_previous=False):
        ...
        predictions, _ = self.decode(init_encoder_outputs, init_attention_masks, (
            encoder_h_state, encoder_c_state), decoder_inputs, feed_previous)
    ...
    def get_batch(self, data, start_idx):
        ...
        for i in xrange(self.batch_size):
            if i + start_idx < len(data):
                ...
                _, _, encoder_manager, decoder_input = data[i +
                                                            start_idx - len(data)]
